  ...
  return (
    <div className="relative min-h-screen w-full flex flex-col items-center justify-start overflow-x-hidden">
      ...
      <div className="relative z-10 w-full flex flex-col items-center mt-24 mb-8">
        <h1 className="text-3xl sm:text-4xl font-bold text-white drop-shadow-lg mb-6 tracking-tight">Search Flights</h1>
      </div>
      {/* Search Bar */}
      ...
      {showResults && (
        <div className="relative z-10 w-full max-w-4xl px-0 sm:px-2 mb-16 animate-fadein">
          <div className="bg-white/10 backdrop-blur-md rounded-3xl shadow-2xl p-2 sm:p-6 border border-zinc-700">
            <h3 className="text-xl sm:text-2xl font-bold text-white mb-2 sm:mb-4">Available Flights</h3>
            <div className="overflow-x-auto rounded-xl">
              <table className="min-w-full text-white text-center text-xs sm:text-base">
                <thead>
                  <tr>
                    <th className="py-2 px-2 sm:px-4 text-blue-300">From</th>
                    <th className="py-2 px-2 sm:px-4 text-blue-300">To</th>
                    <th className="py-2 px-2 sm:px-4 text-blue-300">Date</th>
                    <th className="py-2 px-2 sm:px-4 text-blue-300">Time</th>
                    <th className="py-2 px-2 sm:px-4 text-blue-300">Airline</th>
                    <th className="py-2 px-2 sm:px-4 text-blue-300">Price</th>
                    <th className="py-2 px-2 sm:px-4 text-blue-300">Action</th>
                  </tr>
                </thead>
                <tbody>
                  {filteredFlights.length === 0 ? (
                    <tr>
                      <td colSpan={7} className="py-8 text-zinc-400 text-lg">
                        No flights found for your search.
                      </td>
                    </tr>
                  ) : (
                    filteredFlights.map(f => (
                      <tr key={f.id} className="hover:bg-blue-900/20 transition">
                        <td className="py-2 px-2 sm:px-4">{f.from}</td>
                        <td className="py-2 px-2 sm:px-4">{f.to}</td>
                        <td className="py-2 px-2 sm:px-4">{f.date}</td>
                        <td className="py-2 px-2 sm:px-4">{f.time}</td>
                        <td className="py-2 px-2 sm:px-4">{f.airline}</td>
                        <td className="py-2 px-2 sm:px-4">{f.price}</td>
