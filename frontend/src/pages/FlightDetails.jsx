...
async function fetchCityImage(city) {
  try {
    const res = await fetch(
      `https://en.wikipedia.org/w/api.php?action=query&format=json&origin=*&prop=pageimages&titles=${encodeURIComponent(city)}&pithumbsize=1200`
    );
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-screen w-full bg-black/80 text-white">
        <div className="bg-white/10 rounded-2xl shadow-xl p-8 max-w-lg w-full flex flex-col items-center gap-4">
          <h2 className="text-3xl font-bold mb-2">Flight Not Found</h2>
          <p className="text-zinc-300">Sorry, we couldn't find this flight.</p>
          <Link to="/search-flights" className="mt-4 text-blue-400 hover:underline">Back to Search</Link>
        </div>
      </div>
    );
  }
    ...
    <div className="relative min-h-screen w-full flex flex-col items-center justify-center overflow-x-hidden">
      ...
      <div className="relative z-10 flex flex-col items-center justify-center min-h-[60vh] w-full">
        <div className=" backdrop-blur-md rounded-3xl shadow-2xl p-8 max-w-xl w-full flex flex-col items-center border-4 border-dashed border-blue-400 animate-fadein">
          <h2 className="text-3xl font-extrabold text-white mb-4 tracking-wider">Electronic Ticket</h2>
          <div className="w-full flex flex-col gap-4 text-lg text-white">
            <div className="flex justify-between items-center w-full">
              <span className="font-bold text-blue-300">From:</span>
              <span className="font-mono text-xl">{flight.from}</span>
            </div>
            <div className="flex justify-between items-center w-full">
              <span className="font-bold text-purple-300">To:</span>
              <span className="font-mono text-xl">{flight.to}</span>
            </div>
            <div className="flex justify-between items-center w-full">
              <span className="font-bold text-blue-200">Date:</span>
              <span className="font-mono">{flight.date}</span>
            </div>
            <div className="flex justify-between items-center w-full">
              <span className="font-bold text-purple-200">Time:</span>
              <span className="font-mono">{flight.time}</span>
            </div>
            <div className="flex justify-between items-center w-full">
              <span className="font-bold text-blue-100">Airline:</span>
              <span className="font-mono">{flight.airline}</span>
            </div>
            <div className="flex justify-between items-center w-full">
              <span className="font-bold text-green-300">Price:</span>
              <span className="font-mono text-2xl">{flight.price}</span>
            </div>
            <div className="flex justify-between items-center w-full mt-2">
              <span className="font-bold text-zinc-300">Ticket ID:</span>
              <span className="font-mono text-zinc-200">{flight.id}</span>
            </div>
          </div>
          <div className="w-full border-t border-dashed border-blue-400 my-6" />
          <div className="flex flex-col items-center gap-2">
            <span className="text-zinc-300 text-sm">Enjoy your trip to <span className="text-blue-200 font-bold">{flight.to}</span>!</span>
            <Link to="/search-flights" className="mt-2 px-6 py-2 rounded-full bg-gradient-to-r from-blue-900 to-purple-900 text-white font-bold shadow hover:scale-105 transition">Back to Search</Link>
          </div>
        </div>
      </div>
      <style>{`
